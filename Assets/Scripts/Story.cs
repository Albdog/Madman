...
public class Story : MonoBehaviour {
    ...
    void OpeningScene() {
        ...
        text.text = "Greetings. I'm John and I welcome you to Smart Labs. We're currently located in one of our remote testing sites and you're trying out our latest simulation on schizophrenic symptoms for a horror game we're working on. It's still a prototype, so we need to test out a bunch of features with a select few for our next iteration.";
        if(time > 20f) text.text = "Anyway, that's enough with introductions. I know you're eager to dive in already. Give me a second to boot up the simulation, this won't take too long. ";
        if(time > 29f) text.text = "Now, while our devices are still booting up, I guess I should talk to you first about schizophrenia, since this is a simulation.";
        if(time > 52f) text.text = "I'm sure you're well aware that patients afflicted by schizophrenia have problems understanding reality. Sometimes, they see things that aren't really there, and that scares them. Other times, they start believing false things, like them being hunted by a secret society. What most people seem to forget is that they're people who are suffering, just like people with appendicitis or cancer.";
        if(time > 57f) text.text = "Patients with schizophrenia experience positive and negative symptoms.";
        if(time > 71f) text.text = "Positive Symptoms:\n" + "Hallucinations, delusions, formal thought disorder, and abnormalities and motor behavior represent excessive or distorted bodily functions, distorting one's sense of reality";
        if(time > 86f) text.text = "Negative Symptoms:\n" + "Alogia, affective flattening, anhedonia, and anosgnosia essentially make a person function regressively";
        if(time > 100f) text.text = "It is important to understand that not all patients have the same symptoms. What you'll be experiencing in this simulation is merely a small set of the positive symptoms a person with schizophrenia may experience.";
        if(time > 119f) text.text = "Now, patients with schizophrenia also have other symptoms that affect their way of thinking, but these are hard to simulate without directly messing with your brain. So don't worry, you shouldn't experience any aftereffects at all after this experiment. As for the scary stuff... well, you'll know when you see it. Don't worry, they can't hurt you.";
        if(time > 122f) SceneManager.LoadScene(1);
    }
